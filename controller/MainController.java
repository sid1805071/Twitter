...
public class MainController extends BaseController {
    ...
    SignUpResponse signUpData(@RequestBody Member member, HttpServletRequest request, HttpServletResponse response) {
        ...
        {
             message ="User already created over for this email id !";
        }else {
            member.role="cm";
    ...
    public String getGoogle(ModelMap modelMap,HttpServletResponse response, HttpServletRequest request) {
        String search = "";
        Google google = new Google(search);
        try {
            google.getSuggestions();
        } catch (Exception e) {
            e.printStackTrace();
        }
        return "google";
